...
export default function TollImport() {
  const [step, setStep] = useState<ImportStep>("upload");
  const [vendor, setVendor] = useState<string>("ezpass");
  ...
  const importMutation = trpc.fuelImport.importToll.useMutation();

  // Parse EZPass CSV format
  const parseEZPassCSV = (csvText: string): ParsedTransaction[] => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      // Map EZPass CSV columns to our schema
      const txnId = row['transaction id'] || row['txn id'] || row['id'] || row['posting id'] || `TOLL-${Date.now()}-${i}`;
      const dateStr = row['date'] || row['transaction date'] || row['posting date'] || row['exit date'];
      const amountStr = row['amount'] || row['toll'] || row['toll amount'] || row['charge'];
      const transponder = row['transponder'] || row['tag'] || row['tag number'] || row['ezpass tag'];
      const plate = row['license plate'] || row['plate'] || row['vehicle plate'];
      const unit = row['unit'] || row['unit number'] || row['vehicle'];
      const plaza = row['plaza'] || row['toll plaza'] || row['location'];
      const road = row['road'] || row['toll road'] || row['highway'];
      const entry = row['entry'] || row['entry plaza'] || row['entry point'];
      const exit = row['exit'] || row['exit plaza'] || row['exit point'];
      ...
      transactions.push({
        ...
        amount,
        transponderNumber: transponder || undefined,
        licensePlate: plate || undefined,
        unitNumber: unit || undefined,
        tollPlaza: plaza || undefined,
        tollRoad: road || undefined,
        entryPlaza: entry || undefined,
        exitPlaza: exit || undefined,
  ...
  const handleFileUpload = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      const parsed = parseEZPassCSV(text);
    ...
    try {
      const result = await importMutation.mutateAsync({
        ...
        transactions: validTransactions.map(t => ({
          ...
          tollRoad: t.tollRoad,
          entryPlaza: t.entryPlaza,
          exitPlaza: t.exitPlaza,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Import toll transactions from vendor CSV files
          </p>
        </div>
      </div>
      ...
      {step === "upload" && (
        <Card>
          <CardHeader>
            <CardTitle>Upload Toll CSV</CardTitle>
            <CardDescription>
              Select your toll vendor and upload the CSV export file
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-2">
              <Label>Toll Vendor</Label>
              <Select value={vendor} onValueChange={setVendor}>
                <SelectTrigger className="w-64">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="ezpass">EZPass</SelectItem>
            ...
            <Alert>
              ...
              <AlertDescription>
                The CSV should include columns for: Transaction ID, Date, Amount, Transponder Number, License Plate, Toll Plaza, Toll Road
              </AlertDescription>
            </Alert>
          </CardContent>
        </Card>
                  ...
                  <TableBody>
                    {parsedData.slice(0, 50).map((txn, idx) => (
                      <TableRow key={idx} className={!txn.isValid ? "bg-red-50" : ""}>
                        ...
                        <TableCell>{txn.transponderNumber || "-"}</TableCell>
                        <TableCell>{txn.licensePlate || "-"}</TableCell>
                        <TableCell>{txn.tollPlaza || "-"}</TableCell>
                        <TableCell>{txn.tollRoad || "-"}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
        ...
        <Card>
          <CardContent className="py-12">
            <div className="text-center space-y-4">
              ...
              <p className="text-muted-foreground">
                Processing {validCount} toll transactions
              </p>
            </div>
          </CardContent>
        </Card>
